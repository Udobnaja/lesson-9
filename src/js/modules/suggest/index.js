...
const templateUser = '<p class="log__user">Root: ~ User$</p>';
...
export class Autocomplete {
    ...
    _filterSlicedData(value, event) {
        ...
        if (key === 8) {
            ...
            let i = (stackLength > 1) ?
                this.indexStack[stackLength] - this.indexStack[stackLength - 1] :
                this.indexStack[stackLength] | 0;

            data = [...this.data.slice(i)];
        } else {
            data = [...this.filteredData,
                ...this.data.slice((stackLength) ? this.indexStack[stackLength - 1] : 0)];
        }

        let index = (this.filteredData.length > 0 && this.filteredData.length < this.limit) ?
            this.limit :
            this.filteredData.length;

        for (const street of data) {
            if (streets.length >= this.limit) {
                break;
            }
        ...
        log.innerHTML += `
            Поиск 
            <span style="color: blueviolet"> slice </span> 
            улиц занял 
            <span style="color: red">${(end - start).toFixed(2)}</span> 
            мс 
            <br> 
            ${templateUser}
        `;
